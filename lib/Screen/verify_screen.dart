...
import '../ApiService/api_service.dart';
import '../Modal/verify_otp_modal.dart';
...
class VerifyScreen extends StatefulWidget {
  ...
  final String? verificationId;
  final CountryCode? selectedCountry;
  static const verifyId = 'VerifyPhoneNumberScreen';
  const VerifyScreen(
      {super.key, this.phoneNo, this.verificationId, this.selectedCountry});
  @override
  State<VerifyScreen> createState() => _VerifyScreenState();
...
class _VerifyScreenState extends State<VerifyScreen>
  ...
  Widget build(BuildContext context) {
    return FirebasePhoneAuthHandler(
        ...
        onCodeSent: () {
          debugPrint('OTP sent! ${VerifyScreen.verifyId}');
        },
        onLoginSuccess: (userCredential, autoVerified) async {
          log(
            VerifyScreen.verifyId,
            name: autoVerified
        ...
        onLoginFailed: (authException, stackTrace) {
          ...
          switch (authException.code) {
            case 'invalid-phone-number':
              ...
              return Helper().showToast('Invalid phone number!');

            case 'invalid-verification-code':
              // invalid otp entered
              return Helper().showToast('The entered OTP is invalid!');
        ...
        builder: (context, controller) {
          return Scaffold(
            ...
            body: SingleChildScrollView(
              child: Padding(
                padding:
                    const EdgeInsets.symmetric(horizontal: 15, vertical: 37),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    InkWell(
                    ...
                    RichText(
                      text: TextSpan(
                        text: AppStrings.codeVerification,
                        style:
                            AppFonts.text1.copyWith(color: AppColor.lightWhite),
                        children: <InlineSpan>[
                          TextSpan(
                            text: AppStrings.yourNumber,
